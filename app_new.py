    ...
    html.Div([
        ...
        dcc.Tabs([
            dcc.Tab(children=[ 
                 ...
                 html.Br(),
                 html.Br(),
                 html.Div(children = 'Date chosen: (for debugging)*',style={'fontSize': 20}), # Visual feedback on page displaying date
                ...
                html.Div(children=[
                    html.Div(children='Generation by source',style={'fontSize': 22,'font-weight':'bold','textAlign':'center'}),    
                    html.Div(dcc.Graph(id='gen_pie_daily'),style={'display': 'inline-block', 'width': '100%', 'textAlign': 'center'}),
                                ],
                 ...
                 #This Div is a graph with title, left adjusted beside the following Div
                 html.Div(children=[
                     html.Div(children='Hourly Marginal Price',style={'fontSize': 22,'font-weight':'bold','textAlign':'center'}),
                 ...
                 #This Div is a graph with title, right adjusted beside the previous Div
                 html.Div(children=[
                     html.Div(children='Hourly Energy Storage Charge/Discharge',style={'fontSize': 22,'font-weight':'bold','textAlign':'center'}),
                     html.Div(dcc.Graph(id='storage_volume'),
                              style={'display': 'inline-block', 'width': '100%', 'textAlign': 'center'})
                                     ],
                                     style={'display':'inline-block','width': '50%', 'verticalAlign': 'top', 'textAlign': 'center'}),
                 html.Div(children=[
                     html.Div(children='Emission Intensity of Generated Electricity (tonsCO2e/MWh)',style={'fontSize': 22,'font-weight':'bold','textAlign':'center'}),
                 ...
                 #Reference Tables
                 html.Div(children ='Hourly Statistics',style={'fontSize': 32,'font-weight':'bold','textAlign':'center'}),
                 html.Br(),
        ...
        dcc.Tab(children=[
            ...
            html.Br(),
            html.Div(children = 'Dates chosen: (for debugging)*',style={'fontSize': 20}), # Visual feedback on page displaying date
            dcc.Markdown(id='date_range_display',children = ''),
            html.Br(),
            
            html.Div(children=[
                html.Div(children='Generation by asset type',style={'fontSize': 22,'font-weight':'bold','textAlign':'center'}),
           ...
           html.Div(children=[
               html.Div(children='% generation by asset type',style={'fontSize': 22,'font-weight':'bold','textAlign':'center'}),    
               html.Div(dcc.Graph(id='gen_pie_multi'),style={'display': 'inline-block', 'width': '100%', 'textAlign': 'center'}),
                           ],
                           style={'display':'inline-block','width': '30%', 'verticalAlign': 'top', 'textAlign': 'center'}),
...
def update_date(user_selected):    
    ...
    asset_gen_fig = px.area(df_daily[0], x='Date (MST)', y="Volume", color="Fuel Type",labels={"y": "Generation Volume MWh"})
    asset_gen_fig.update_layout(yaxis_title='Generation Volume (MW)')
    clean_ff_fig = px.line(df_daily[1],x='Date (MST)',y=['Total Load','Net Load'],color_discrete_sequence=['black', 'gray'],range_y=[0,None],labels={"y": "Generation Volume MWh"})
    ...
    EI_fig = px.scatter(df_daily[1],x='Date (MST)',y='RE_percent',color = 'EI', color_continuous_scale=['green', 'yellow', '#D62728'])
    EI_fig.update_layout(yaxis_title='% Generation from Renewables')
    gen_pie_daily = px.pie(df_daily[0], values='Volume', names='Fuel Type') #title='% Generation over timeframe')
    return user_selected, summary_data, percent_data, asset_gen_fig, clean_ff_fig, price_fig, storage_fig, EI_fig, gen_pie_daily # The returned object is assigned to the component property of the Output
...
def update_output(start_date, end_date):
    ...
    if start_date <= end_date:
        ...
        gen_multi = px.area(df_multi[1], x='Date (MST)', y="Volume", color="Fuel Type",labels={"y": "Generation Volume MWh"})
        gen_pie_multi = px.pie(df_multi[1], values='Volume', names='Fuel Type')#, title='% Generation over timeframe')
    else:
        gen_multi = 'LOADING...'
